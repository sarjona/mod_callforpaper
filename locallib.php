...
/**
 * The class to handle entry exports of a callforpaper module
 */
class callforpaper_portfolio_caller extends portfolio_module_caller_base {

    /** @var int the single record to export */
    protected $recordid;

    /** @var object the record from the callforpaper table */
    ...
    /**
     * load up the callforpaper needed for the export
     *
     * @global object $DB
     */
    public function load_data() {
        global $DB, $USER;
        if (!$this->cm = get_coursemodule_from_id('callforpaper', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'callforpaper');
        }
        if (!$this->callforpaper = $DB->get_record('callforpaper', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'callforpaper');
        }
        $fieldrecords = $DB->get_records('callforpaper_fields', array('callforpaperid' => $this->cm->instance), 'id');
        // populate objets for this callforpapers fields
        $this->fields = array();
        foreach ($fieldrecords as $fieldrecord) {
            $tmp = callforpaper_get_field($fieldrecord, $this->callforpaper);
            $this->fields[] = $tmp;
        ...
        $this->records = array();
        if ($this->recordid) {
            $tmp = $DB->get_record('callforpaper_records', array('id' => $this->recordid));
            $tmp->content = $DB->get_records('callforpaper_content', array('recordid' => $this->recordid));
            $this->records[] = $tmp;
        } else {
            $where = array('callforpaperid' => $this->callforpaper->id);
            if (!has_capability('mod/callforpaper:exportallentries', context_module::instance($this->cm->id))) {
                $where['userid'] = $USER->id; // get them all in case, we'll unset ones that aren't ours later if necessary
            }
            $tmp = $DB->get_records('callforpaper_records', $where);
            foreach ($tmp as $t) {
                $t->content = $DB->get_records('callforpaper_content', array('recordid' => $t->id));
                $this->records[] = $t;
            }
            $this->minecount = $DB->count_records('callforpaper_records', array('callforpaperid' => $this->callforpaper->id, 'userid' => $USER->id));
        }
    ...
    public function get_sha1() {
        ...
        // then we're not exporting any metacallforpaper, just the file by itself by mimetype.
        if ($this->exporter->get('format') instanceof portfolio_format_file && $this->singlefile) {
            return $this->get_sha1_file();
        }
        // otherwise we're exporting some sort of multipart content so use the callforpaper
        $str = '';
        foreach ($this->records as $record) {
            foreach ($record as $callforpaper) {
                if (is_array($callforpaper) || is_object($callforpaper)) {
                    $keys = array_keys($callforpaper);
                    $testkey = array_pop($keys);
                    if (is_array($callforpaper[$testkey]) || is_object($callforpaper[$testkey])) {
                        foreach ($callforpaper as $d) {
                            $str .= implode(',', (array)$d);
                        }
                    } else {
                        $str .= implode(',', (array)$callforpaper);
                    }
                } else {
                    $str .= $callforpaper;
                }
    ...
    public function prepare_package() {
        ...
        foreach ($this->records  as $key => $record) {
            ...
            $content .= $tmpcontent;
            if ($leapwriter) {
                $entry = new portfolio_format_leap2a_entry('callforpaperentry' . $record->id, $this->callforpaper->name, 'resource', $tmpcontent);
                ...
                $ids[] = $entry->id;
                $leapwriter->link_files($entry, $files, 'callforpaperentry' . $record->id . 'file');
                $leapwriter->add_entry($entry);
            }
        }
        if ($leapwriter) {
            if (count($this->records) > 1) { // make a selection element to tie them all together
                $selection = new portfolio_format_leap2a_entry('callforpaperdb' . $this->callforpaper->id,
                    get_string('entries', 'callforpaper') . ': ' . $this->callforpaper->name, 'selection');
    ...
    public function check_permissions() {
        if ($this->recordid) {
            if (callforpaper_isowner($this->recordid)) {
                return has_capability('mod/callforpaper:exportownentry', context_module::instance($this->cm->id));
            }
            return has_capability('mod/callforpaper:exportentry', context_module::instance($this->cm->id));
        }
        if ($this->has_export_config() && !$this->get_export_config('mineonly')) {
            return has_capability('mod/callforpaper:exportallentries', context_module::instance($this->cm->id));
        }
        return has_capability('mod/callforpaper:exportownentry', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'callforpaper');
    }
    ...
    public function __wakeup() {
        ...
        foreach ($this->fieldtypes as $key => $field) {
            $filepath = $CFG->dirroot . '/mod/callforpaper/field/' . $field .'/field.class.php';
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        ...
        if (empty($this->callforpaper->singletemplate)) {
            // Use default template if the template is not created.
            $this->callforpaper->singletemplate = callforpaper_generate_default_template($this->callforpaper, 'singletemplate', 0, false, false);
        }

        // actual replacement of the tags
        return array(str_ireplace($patterns, $replacement, $this->callforpaper->singletemplate), $files);
    }
     ...
     * @param object $record The callforpaper record object
     *
    ...
    public static function has_files($callforpaper) {
        global $DB;
    ...
    public function has_export_config() {
        ...
        return (empty($this->recordid) // multi-entry export
            ...
            && has_capability('mod/callforpaper:exportallentries', context_module::instance($this->cm->id))); // they actually have a choice in the matter
    }
    ...
    public function export_config_form(&$mform, $instance) {
        ...
        $mform->addElement('selectyesno', 'mineonly', get_string('exportownentries', 'callforpaper', (object)array('mine' => $this->minecount, 'all' => count($this->records))));
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class callforpaper_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    public function __construct($browser, $course, $cm, $context, $areas, $filearea) {
        ...
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_callforpaper';
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_callforpaper', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
...
/**
 * This creates new calendar events given as timeavailablefrom and timeclose by $callforpaper.
 *
 * @param stdClass $callforpaper
 * @return void
 */
function callforpaper_set_events($callforpaper) {
    global $DB, $CFG;

    require_once($CFG->dirroot.'/calendar/lib.php');

    // Get CMID if not sent as part of $callforpaper.
    if (!isset($callforpaper->coursemodule)) {
        $cm = get_coursemodule_from_instance('callforpaper', $callforpaper->id, $callforpaper->course);
        $callforpaper->coursemodule = $cm->id;
    }
    // Call for paper start calendar events.
    $event = new stdClass();
    $event->eventtype = CALLFORPAPER_EVENT_TYPE_OPEN;
    // The CALLFORPAPER_EVENT_TYPE_OPEN event should only be an action event if no close time was specified.
    $event->type = empty($callforpaper->timeavailableto) ? CALENDAR_EVENT_TYPE_ACTION : CALENDAR_EVENT_TYPE_STANDARD;
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'callforpaper', 'instance' => $callforpaper->id, 'eventtype' => $event->eventtype))) {
        if ($callforpaper->timeavailablefrom > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarstart', 'callforpaper', $callforpaper->name);
            $event->description  = format_module_intro('callforpaper', $callforpaper, $callforpaper->coursemodule, false);
            $event->format       = FORMAT_HTML;
            $event->timestart    = $callforpaper->timeavailablefrom;
            $event->timesort     = $callforpaper->timeavailablefrom;
            $event->visible      = instance_is_visible('callforpaper', $callforpaper);
        ...
        if (isset($callforpaper->timeavailablefrom) && $callforpaper->timeavailablefrom > 0) {
            $event->name         = get_string('calendarstart', 'callforpaper', $callforpaper->name);
            $event->description  = format_module_intro('callforpaper', $callforpaper, $callforpaper->coursemodule, false);
            $event->format       = FORMAT_HTML;
            $event->courseid     = $callforpaper->course;
            $event->groupid      = 0;
            $event->userid       = 0;
            $event->modulename   = 'callforpaper';
            $event->instance     = $callforpaper->id;
            $event->timestart    = $callforpaper->timeavailablefrom;
            $event->timesort     = $callforpaper->timeavailablefrom;
            $event->visible      = instance_is_visible('callforpaper', $callforpaper);
            $event->timeduration = 0;
    ...
    // Call for paper end calendar events.
    $event = new stdClass();
    $event->type = CALENDAR_EVENT_TYPE_ACTION;
    $event->eventtype = CALLFORPAPER_EVENT_TYPE_CLOSE;
    if ($event->id = $DB->get_field('event', 'id',
            array('modulename' => 'callforpaper', 'instance' => $callforpaper->id, 'eventtype' => $event->eventtype))) {
        if ($callforpaper->timeavailableto > 0) {
            // Calendar event exists so update it.
            $event->name         = get_string('calendarend', 'callforpaper', $callforpaper->name);
            $event->description  = format_module_intro('callforpaper', $callforpaper, $callforpaper->coursemodule, false);
            $event->format       = FORMAT_HTML;
            $event->timestart    = $callforpaper->timeavailableto;
            $event->timesort     = $callforpaper->timeavailableto;
            $event->visible      = instance_is_visible('callforpaper', $callforpaper);
            $event->timeduration = 0;
            $calendarevent = calendar_event::load($event->id);
            $calendarevent->update($event, false);
        ...
        // Event doesn't exist so create one.
        if (isset($callforpaper->timeavailableto) && $callforpaper->timeavailableto > 0) {
            $event->name         = get_string('calendarend', 'callforpaper', $callforpaper->name);
            $event->description  = format_module_intro('callforpaper', $callforpaper, $callforpaper->coursemodule, false);
            $event->format       = FORMAT_HTML;
            $event->courseid     = $callforpaper->course;
...
/**
 * Check if a callforpaper is available for the current user.
 *
 * @param  stdClass  $callforpaper           callforpaper record
 ...
 * @since  Moodle 3.3
 */
function callforpaper_get_time_availability_status($callforpaper, $canmanageentries = null, $context = null) {
    $open = true;
    $closed = false;
    $warnings = array();
    ...
    if (!$canmanageentries) {
        ...
        if (!empty($callforpaper->timeavailablefrom) and $callforpaper->timeavailablefrom > $timenow) {
            $open = false;
        }
        if (!empty($callforpaper->timeavailableto) and $timenow > $callforpaper->timeavailableto) {
            $closed = true;
        }

        if (!$open or $closed) {
            if (!$open) {
                $warnings['notopenyet'] = userdate($callforpaper->timeavailablefrom);
            }
            if ($closed) {
                $warnings['expired'] = userdate($callforpaper->timeavailableto);
            }
            return array(false, $warnings);
        }
    }

    // Call for paper is available.
    return array(true, $warnings);
}

/**
 * Requires a callforpaper to be available for the current user.
 *
 * @param  stdClass  $callforpaper           callforpaper record
 * @param  boolean $canmanageentries  optional, if the user can manage entries
 * @param  stdClass  $context          Module context, required if $canmanageentries is not set
 ...
 * @since  Moodle 3.3
 */
function callforpaper_require_time_available($callforpaper, $canmanageentries = null, $context = null) {

    list($available, $warnings) = callforpaper_get_time_availability_status($callforpaper, $canmanageentries, $context);

    if (!$available) {
        $reason = current(array_keys($warnings));
        throw new moodle_exception($reason, 'callforpaper', '', $warnings[$reason]);
    }
}
 ...
 *
 * @param  stdClass $callforpaper          callforpaper object
 ...
 * @since  Moodle 3.3
 */
function callforpaper_get_entries_left_to_add($callforpaper, $numentries, $canmanageentries) {
    if ($callforpaper->requiredentries > 0 && $numentries < $callforpaper->requiredentries && !$canmanageentries) {
        return $callforpaper->requiredentries - $numentries;
    }
    return 0;
}
 ...
 * @since  Moodle 3.3
 */
function callforpaper_get_entries_left_to_view($callforpaper, $numentries, $canmanageentries) {
    if ($callforpaper->requiredentriestoview > 0 && $numentries < $callforpaper->requiredentriestoview && !$canmanageentries) {
        return $callforpaper->requiredentriestoview - $numentries;
    }
    return 0;
}

/**
 * Returns callforpaper records tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_callforpaper/callforpaper_records to search for callforpaper records
 ...
 */
function mod_callforpaper_get_tagged_records($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    global $DB, $OUTPUT, $USER;
    $perpage = $exclusivemode ? 20 : 5;

    // Build the SQL query.
    $ctxselect = context_helper::get_preload_record_columns_sql('ctx');
    $query = "SELECT dr.id, dr.callforpaperid, dr.approved, d.timeviewfrom, d.timeviewto, dr.groupid, d.approval, dr.userid,
                     d.requiredentriestoview, cm.id AS cmid, c.id AS courseid, c.shortname, c.fullname, $ctxselect
                FROM {callforpaper_records} dr
                JOIN {callforpaper} d
                  ON d.id = dr.callforpaperid
                JOIN {modules} m
                  ON m.name = 'callforpaper'
    ...
    $builder = new core_tag_index_builder('mod_callforpaper', 'callforpaper_records', $query, $params, $page * $perpage, $perpage + 1);
    $now = time();
    $entrycount = [];
    $activitygroupmode = [];
    $usergroups = [];
    ...
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        if ($item->requiredentriestoview) {
            if (!isset($entrycount[$item->callforpaperid])) {
                $entrycount[$item->callforpaperid] = $DB->count_records('callforpaper_records', array('callforpaperid' => $item->callforpaperid));
            }
            $sufficiententries = $item->requiredentriestoview > $entrycount[$item->callforpaperid];
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $modinfo = get_fast_modinfo($item->courseid);
            $cm = $modinfo->get_cm($item->cmid);
            $pageurl = new moodle_url('/mod/callforpaper/view.php', array(
                    'rid' => $item->id,
                    'd' => $item->callforpaperid
            ));

            if (!isset($titlefields[$item->callforpaperid])) {
                $titlefields[$item->callforpaperid] = callforpaper_get_tag_title_field($item->callforpaperid);
            }

            $pagename = callforpaper_get_tag_title_for_entry($titlefields[$item->callforpaperid], $item);
        ...
        return new core_tag\output\tagindex($tag, 'mod_callforpaper', 'callforpaper_records', $content, $exclusivemode,
 ...
 *
 * @param int $callforpaperid The id of the callforpaper field
 * @return stdClass The field callforpaper from the 'callforpaper_fields' table as well as it's priority
 */
function callforpaper_get_tag_title_field($callforpaperid) {
    global $DB, $CFG;

    $validfieldtypes = array('text', 'textarea', 'menu', 'radiobutton', 'checkbox', 'multimenu', 'url');
    $fields = $DB->get_records('callforpaper_fields', ['callforpaperid' => $callforpaperid]);
    $template = $DB->get_field('callforpaper', 'addtemplate', ['id' => $callforpaperid]);
    if (empty($template)) {
        $callforpaper = $DB->get_record('callforpaper', ['id' => $callforpaperid]);
        $template = callforpaper_generate_default_template($callforpaper, 'addtemplate', 0, false, false);
    }
    ...
    $sort = function($record1, $record2) {
        // If a content's fieldtype is compulsory in the callforpaper than it would have priority than any other non-compulsory content.
 ...
 *
 * @param stdClass $field The field from the 'callforpaper_fields' table
 * @param stdClass $entry The entry from the 'callforpaper_records' table
 * @return string|null It will return the title of the entry or null if the field type is not available.
 */
function callforpaper_get_tag_title_for_entry($field, $entry) {
    global $CFG, $DB;
    ...
    $classname = 'callforpaper_field_' . $field->type;
    $sql = "SELECT dc.*
              FROM {callforpaper_content} dc
        INNER JOIN {callforpaper_fields} df
                ON dc.fieldid = df.id
             WHERE df.id = :fieldid
...
/**
 * Search entries in a callforpaper.
 *
 * @param  stdClass  $callforpaper        callforpaper object
 * @param  stdClass  $cm           course module object
 * @param  stdClass  $context      context object
 * @param  stdClass  $mode         in which mode we are viewing the callforpaper (list, single)
 * @param  int  $currentgroup      the current group being used
 * @param  str  $search            search for this text in the entry callforpaper
 ...
 * @param  array  $searcharray     when using advanced search, the advanced callforpaper to use
 * @param  stdClass  $record       if we jsut want this record after doing all the access checks
 * @return array the entries found among other callforpaper related to the search
 * @since  Moodle 3.3
 */
function callforpaper_search_entries($callforpaper, $cm, $context, $mode, $currentgroup, $search = '', $sort = null, $order = null, $page = 0,
        $perpage = 0, $advanced = null, $searcharray = null, $record = null) {
    global $DB, $USER;

    if ($sort === null) {
        $sort = $callforpaper->defaultsort;
    }
    if ($order === null) {
        $order = ($callforpaper->defaultsortdir == 0) ? 'ASC' : 'DESC';
    }
    if ($searcharray === null) {
        $searcharray = array();
    }
    ...
    if (callforpaper_get_entries_left_to_view($callforpaper, $numentries, $canmanageentries)) {
    ...
    if (!$approvecap && $callforpaper->approval) {
        if (isloggedin()) {
    ...
    if ($sort <= 0 || !($sortfield = callforpaper_get_field_from_id($sort, $callforpaper))) {

        switch ($sort) {
            // SARATODO: Can this be improved?
            case CALLFORPAPER_REVIEWCOUNT:
        ...
        $what = ' DISTINCT r.id, r.approved, r.timecreated, r.timemodified, r.userid, r.groupid, r.callforpaperid, ' . $namefields;
        $count = ' COUNT(DISTINCT c.recordid) ';
        $tables = '{callforpaper_content} c,{callforpaper_records} r, {user} u ';
        $where = 'WHERE c.recordid = r.id
                     AND r.callforpaperid = :callforpaperid
                     AND r.userid = u.id ';
        $params['callforpaperid'] = $callforpaper->id;
    ...
    $recordids = callforpaper_get_all_recordids($callforpaper->id, $initialselect, $initialparams);
    $newrecordids = callforpaper_get_advance_search_ids($recordids, $searcharray, $callforpaper->id);
    $selectcallforpaper = $where . $groupselect . $approveselect;

    if (!empty($advanced)) {
        $advancedsearchsql = callforpaper_get_advanced_search_sql($sort, $callforpaper, $newrecordids, $selectcallforpaper, $sortorder);
    ...
    if (!$records = $DB->get_records_sql($sqlselect, $allparams, $page * $nowperpage, $nowperpage)) {
        ...
        if ($record) {         // Something was requested so try to show that at least (bug 5132)
            if (callforpaper_can_view_record($callforpaper, $record, $currentgroup, $canmanageentries)) {
 ...
 * @since  Moodle 3.3
 */
function callforpaper_can_view_record($callforpaper, $record, $currentgroup, $canmanageentries) {
    global $USER;

    if ($canmanageentries || empty($callforpaper->approval) ||
...
/**
 * Return all the field instances for a given callforpaper.
 *
 * @param  stdClass $callforpaper callforpaper object
 * @return array field instances
 * @since  Moodle 3.3
 */
function callforpaper_get_field_instances($callforpaper) {
    global $DB;

    $instances = [];
    if ($fields = $DB->get_records('callforpaper_fields', array('callforpaperid' => $callforpaper->id), 'id')) {
        foreach ($fields as $field) {
            $instances[] = callforpaper_get_field($field, $callforpaper);
        }
    }
    return $instances;
}
 ...
 *
 * @param  stdClass $callforpaper     the callforpaper object
 ...
 * @since  Moodle 3.3
 */
function callforpaper_build_search_array($callforpaper, $paging, $searcharray, $defaults = null, $fn = '', $ln = '') {
    global $DB;

    $search = '';
    $vals = array();
    $fields = $DB->get_records('callforpaper_fields', array('callforpaperid' => $callforpaper->id));

    if (!empty($fields)) {
        foreach ($fields as $field) {
            $searchfield = callforpaper_get_field_from_id($field->id, $callforpaper);
            // Get field callforpaper to build search sql with.  If paging is false, get from user.
            // If paging is true, get callforpaper from $searcharray which is obtained from the $SESSION (see line 116).
            if (!$paging && $searchfield->type != 'unknown') {
                ...
                if (isset($searcharray[$field->id])) {
                    $val = $searcharray[$field->id]->callforpaper;
            ...
            if (!empty($val)) {
                ...
                $searcharray[$field->id]->callforpaper = $val;
                $vals[] = $val;
            } else {
                // Clear it out.
    ...
    if ($rawtagnames) {
        $searcharray[CALLFORPAPER_TAGS] = new stdClass();
        $searcharray[CALLFORPAPER_TAGS]->params = [];
        $searcharray[CALLFORPAPER_TAGS]->rawtagnames = $rawtagnames;
        $searcharray[CALLFORPAPER_TAGS]->sql = '';
    } else {
        unset($searcharray[CALLFORPAPER_TAGS]);
    }
    ...
    if (!$paging) {
        ...
        $fn = isset($searcharray[CALLFORPAPER_FIRSTNAME]) ? $searcharray[CALLFORPAPER_FIRSTNAME]->callforpaper : '';
        $ln = isset($searcharray[CALLFORPAPER_LASTNAME]) ? $searcharray[CALLFORPAPER_LASTNAME]->callforpaper : '';
    }
    if (!empty($fn)) {
        $searcharray[CALLFORPAPER_FIRSTNAME] = new stdClass();
        $searcharray[CALLFORPAPER_FIRSTNAME]->sql    = '';
        $searcharray[CALLFORPAPER_FIRSTNAME]->params = array();
        $searcharray[CALLFORPAPER_FIRSTNAME]->field  = 'u.firstname';
        $searcharray[CALLFORPAPER_FIRSTNAME]->callforpaper   = $fn;
        $vals[] = $fn;
    } else {
        unset($searcharray[CALLFORPAPER_FIRSTNAME]);
    }
    if (!empty($ln)) {
        $searcharray[CALLFORPAPER_LASTNAME] = new stdClass();
        $searcharray[CALLFORPAPER_LASTNAME]->sql     = '';
        $searcharray[CALLFORPAPER_LASTNAME]->params = array();
        $searcharray[CALLFORPAPER_LASTNAME]->field   = 'u.lastname';
        $searcharray[CALLFORPAPER_LASTNAME]->callforpaper    = $ln;
        $vals[] = $ln;
    } else {
        unset($searcharray[CALLFORPAPER_LASTNAME]);
    }
 ...
 * @since  Moodle 3.3
 */
function callforpaper_approve_entry($entryid, $approve) {
    global $DB;

    $newrecord = new stdClass();
    $newrecord->id = $entryid;
    $newrecord->approved = $approve ? 1 : 0;
    $DB->update_record('callforpaper_records', $newrecord);
}

/**
 * Populate the field contents of a new record with the submitted callforpaper.
 * An event has been previously triggered upon the creation of the new record in callforpaper_add_record().
 *
 * @param  stdClass $callforpaper          callforpaper object
 * @param  stdClass $context        context object
 * @param  int $recordid            the new record id
 * @param  array $fields            list of fields of the callforpaper
 * @param  stdClass $callforpaperrecord     the submitted callforpaper
 * @param  stdClass $processeddata  pre-processed submitted fields
 * @since  Moodle 3.3
 */
function callforpaper_add_fields_contents_to_new_record($callforpaper, $context, $recordid, $fields, $callforpaperrecord, $processeddata) {
    global $DB;
    ...
    // Bulk insert the records now. Some records may have no callforpaper but all must exist.
    $DB->insert_records('callforpaper_content', $records);

    // Add all provided content.
    foreach ($processeddata->fields as $fieldname => $field) {
        $field->update_content($recordid, $callforpaperrecord->$fieldname, $fieldname);
    }
}
 ...
 * @since  Moodle 3.3
 */
function callforpaper_update_record_fields_contents($callforpaper, $record, $context, $callforpaperrecord, $processeddata) {
    global $DB;
    ...
    $DB->update_record('callforpaper_records', $record);

    // Update all content.
    foreach ($processeddata->fields as $fieldname => $field) {
        $field->update_content($record->id, $callforpaperrecord->$fieldname, $fieldname);
    }
    ...
    $event = \mod_callforpaper\event\record_updated::create(array(
        'objectid' => $record->id,
        'context' => $context,
        'courseid' => $callforpaper->course,
        'other' => array(
            'callforpaperid' => $callforpaper->id
        )
    ));
    $event->add_record_snapshot('callforpaper', $callforpaper);
    $event->trigger();
}
