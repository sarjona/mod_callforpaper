...
if (!isset($SESSION->callforpaperprefs)) {
    $SESSION->callforpaperprefs = array();
}
if (!isset($SESSION->callforpaperprefs[$callforpaper->id])) {
    $SESSION->callforpaperprefs[$callforpaper->id] = array();
    $SESSION->callforpaperprefs[$callforpaper->id]['search'] = '';
    $SESSION->callforpaperprefs[$callforpaper->id]['search_array'] = array();
    $SESSION->callforpaperprefs[$callforpaper->id]['sort'] = $callforpaper->defaultsort;
    $SESSION->callforpaperprefs[$callforpaper->id]['advanced'] = 0;
    $SESSION->callforpaperprefs[$callforpaper->id]['order'] = ($callforpaper->defaultsortdir == 0) ? 'ASC' : 'DESC';
}
...
if ($advanced == -1) {
    $advanced = $SESSION->callforpaperprefs[$callforpaper->id]['advanced'];
} else {
    if (!$advanced) {
        // explicitly switched to normal mode - discard all advanced search settings
        $SESSION->callforpaperprefs[$callforpaper->id]['search_array'] = array();
    }
    $SESSION->callforpaperprefs[$callforpaper->id]['advanced'] = $advanced;
}

$search_array = $SESSION->callforpaperprefs[$callforpaper->id]['search_array'];
...
if (!empty($advanced)) {
    ...
    $SESSION->callforpaperprefs[$callforpaper->id]['search_array'] = $search_array;     // Make it sticky.

} else {
    $search = optional_param('search', $SESSION->callforpaperprefs[$callforpaper->id]['search'], PARAM_NOTAGS);
...
$SESSION->callforpaperprefs[$callforpaper->id]['search'] = $search;   // Make it sticky

$sort = optional_param('sort', $SESSION->callforpaperprefs[$callforpaper->id]['sort'], PARAM_INT);
$SESSION->callforpaperprefs[$callforpaper->id]['sort'] = $sort;       // Make it sticky

$order = (optional_param('order', $SESSION->callforpaperprefs[$callforpaper->id]['order'], PARAM_ALPHA) == 'ASC') ? 'ASC': 'DESC';
$SESSION->callforpaperprefs[$callforpaper->id]['order'] = $order;     // Make it sticky


$oldperpage = get_user_preferences('callforpaper_perpage_'.$callforpaper->id, 10);
...
if ($perpage != $oldperpage) {
    set_user_preference('callforpaper_perpage_'.$callforpaper->id, $perpage);
}
...
$pageurl = new moodle_url('/mod/callforpaper/view.php', $urlparams);
...
if ($multidelete && $canmanageentries) {
    if ($confirm) {
        ...
        $serialisedcallforpaper = json_encode($recordids);
        $submitactions = array('d' => $callforpaper->id, 'sesskey' => sesskey(), 'confirm' => '1', 'serialdelete' => $serialisedcallforpaper);
        $action = new moodle_url('/mod/callforpaper/view.php', $submitactions);
        $cancelurl = new moodle_url('/mod/callforpaper/view.php', array('d' => $callforpaper->id));
...
if ($showactivity) {
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($groupmode != NOGROUPS) {
            $returnurl = new moodle_url('/mod/callforpaper/view.php', ['d' => $callforpaper->id, 'mode' => $mode, 'search' => s($search),
        ...
        $actionbar = new \mod_callforpaper\output\action_bar($callforpaper->id, $pageurl);
        ...
        if (empty($records)) {
            ...
            $formurl = new moodle_url('/mod/callforpaper/view.php', ['d' => $callforpaper->id, 'sesskey' => sesskey()]);
            ...
            if ($mode == 'single') { // Single template
                $baseurl = '/mod/callforpaper/view.php';
                ...
                echo $callforpaper->listtemplateheader;
                $options = [
                    'search' => $search,
                    'page' => $page,
                    'baseurl' => $baseurl,
                ...
                echo $callforpaper->listtemplatefooter;
                echo $OUTPUT->box_end();
            }
