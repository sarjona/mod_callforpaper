...
if ($id) {
    ...
    $data = $DB->get_record('callforpaper', ['id' => $d], '*', MUST_EXIST);
...
\mod_callforpaper\local\reviewer_information::pull_data_for($data->id);
...
if ($perpage != $oldperpage) {
    set_user_preference('callforpaper_perpage_'.$data->id, $perpage);
}
...
$meta = '';
if (!empty($CFG->enablerssfeeds) && !empty($CFG->callforpaper_enablerssfeeds) && $data->rssarticles > 0) {
    $rsstitle = $courseshortname . ': ' . format_string($data->name);
    rss_add_http_header($context, 'mod_callforpaper', $data, $rsstitle);
}
if ($data->csstemplate) {
    $PAGE->requires->css('/mod/callforpaper/css.php?d='.$data->id);
}
if ($data->jstemplate) {
    $PAGE->requires->js('/mod/callforpaper/js.php?d='.$data->id, true);
}
...
if ($record and !callforpaper_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
...
/*if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
    ...
    rss_print_link($context->id, $USER->id, 'mod_callforpaper', $data->id, get_string('rsstype'));
    echo '</div>';
...
if ($delete && callforpaper_user_can_manage_entry($delete, $data, $context)) {
    if ($confirm) {
        require_sesskey();

        if (callforpaper_delete_record($delete, $data, $course->id, $cm->id)) {
        ...
        if ($deleterecord = $DB->get_record_sql("SELECT dr.*, $allnamefields
            ...
            if ($deleterecord->callforpaperid == $data->id) {                       // Must be from this callforpaper
                echo $OUTPUT->heading(get_string('deleteentry', 'mod_callforpaper'), 2, 'mb-4');
                $deletebutton = new single_button(
                    new moodle_url('/mod/callforpaper/view.php?d=' . $data->id . '&delete=' . $delete . '&confirm=1'),
...
if ($multidelete && $canmanageentries) {
    if ($confirm) {
        ...
        foreach ($multidelete as $value) {
            callforpaper_delete_record($value, $data, $course->id, $cm->id);
        }
    } else {
        $validrecords = array();
        $recordids = array();
        ...
        $cancelurl = new moodle_url('/mod/callforpaper/reviewview.php', array('d' => $data->id));
...
list($showactivity, $warnings) = callforpaper_get_time_availability_status($data, $canmanageentries);
...
if ($showactivity) {
    ...
    if ($mode == 'asearch') {
        ...
        if (($approve || $disapprove) && $approvecap) {
            ...
            if ($approverecord = $DB->get_record('callforpaper_records', array('id' => $recordid))) {   // Need to check this is valid
                if ($approverecord->callforpaperid == $data->id) {                       // Must be from this callforpaper
        ...
        $numentries = callforpaper_numentries($data);
    /// Check the number of entries required against the number of entries already made (doesn't apply to teachers)
        if ($data->entriesleft = callforpaper_get_entries_left_to_add($data, $numentries, $canmanageentries)) {
            $strentrieslefttoadd = get_string('entrieslefttoadd', 'callforpaper', $data);
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($data->entrieslefttoview = callforpaper_get_entries_left_to_view($data, $numentries, $canmanageentries)) {
            $strentrieslefttoaddtoview = get_string('entrieslefttoaddtoview', 'callforpaper', $data);
            echo $OUTPUT->notification($strentrieslefttoaddtoview);
            $requiredentries_allowed = false;
        }

        if ($groupmode != NOGROUPS) {
            $returnurl = new moodle_url('/mod/callforpaper/reviewview.php', ['d' => $data->id, 'mode' => $mode, 'search' => s($search),
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($data, $cm, $context, $mode, $currentgroup, $search, $sort, $order, $page, $perpage, $advanced, $search_array, $record);
        ...
        if (empty($records)) {
            ...
            $formurl = new moodle_url('/mod/callforpaper/reviewview.php', ['d' => $data->id, 'sesskey' => sesskey()]);
