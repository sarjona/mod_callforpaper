...
use mod_callforpaper_renderer;
use mod_callforpaper\event\course_module_viewed;
use mod_callforpaper\event\template_viewed;
use mod_callforpaper\event\template_updated;
use moodle_page;
use core_component;
use stdClass;

/**
 * Class manager for callforpaper activity
 *
...
class manager {
    ...
    /** Module name. */
    const MODULE = 'callforpaper';

    /** The plugin name. */
    const PLUGINNAME = 'mod_callforpaper';
    ...
    /** @var array the current callforpaper_fields records.
    ...
    /**
     * Create a manager instance from a callforpaper_record entry.
     *
     * @param stdClass $record the callforpaper_record record
     * @return manager
     */
    public static function create_from_callforpaper_record($record): self {
        global $DB;
        $instance = $DB->get_record(self::MODULE, ['id' => $record->callforpaperid], '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance(self::MODULE, $instance->id);
        $cm = cm_info::create($cm);
        return new self($cm, $instance);
    }
     ...
     * @return mod_callforpaper_renderer the module renderer
     */
    public function get_renderer(?moodle_page $page = null): mod_callforpaper_renderer {
        global $PAGE;
        $page = $page ?? $PAGE;
    ...
    /**
     * Return if the callforpaper has records.
     *
     * @return bool true if the callforpaper has records
     */
    public function has_records(): bool {
        global $DB;

        return $DB->record_exists('callforpaper_records', ['callforpaperid' => $this->instance->id]);
    }

    /**
     * Return if the callforpaper has fields.
     *
     * @return bool true if the callforpaper has fields
     */
    public function has_fields(): bool {
        global $DB;
        if ($this->_fieldrecords === null) {
            return $DB->record_exists('callforpaper_fields', ['callforpaperid' => $this->instance->id]);
        }
        return !empty($this->_fieldrecords);
    }

    /**
     * Return the callforpaper entries.
     *
     * @param array $groups to filter by.
     * @return [] the callforpaper records array.
     */
    public function get_all_entries(array $groups = []): array {
        global $DB;

        if (empty($groups)) {
            return $DB->get_records('callforpaper_records', ['callforpaperid' => $this->instance->id]);
        } else {
            [$sql, $params] = $DB->get_in_or_equal(array_keys($groups), SQL_PARAMS_NAMED);
            $sql = 'callforpaperid = :id AND (groupid ' . $sql . ' OR groupid = 0)';
            $params['id'] = $this->instance->id;
            return $DB->get_records_select('callforpaper_records', $sql, $params);
        }
    }

    /**
     * Return the callforpaper given entries filtered by userid.
     *
     * @param array $entries Entries to filter from.
     * @param int $userid User to filter by. Zero for non-filtering.
     *
     * @return [] the filtered callforpaper records array.
     */
    ...
    /**
     * Return the callforpaper comments filtered by approved entries.
     *
     * @param ?int $approved Approved value to filter by. Null for not filtering.
     * @param ?array $groups to filter by.
     *
     * @return [] the filtered callforpaper comments array or null if there is no comment.
     */
    public function get_comments(?int $approved = null, array $groups = []): ?array {
        ...
        $args->cm = $this->cm;
        $args->area = 'callforpaper_entry';
        $args->component = 'mod_callforpaper';
    ...
    /**
     * Return the callforpaper fields.
     *
     * @return callforpaper_field_base[] the field instances.
     */
    ...
    /**
     * Return the field records (the current callforpaper_fields records).
     *
    ...
    public function get_field_records() {
        global $DB;
        if ($this->_fieldrecords === null) {
            $this->_fieldrecords = $DB->get_records('callforpaper_fields', ['callforpaperid' => $this->instance->id], 'id');
        }
     ...
     * @return callforpaper_field_base the callforpaper field class instance
     */
    public function get_field(stdClass $fieldrecord): callforpaper_field_base {
        ...
        $classname = "callforpaper_field_{$fieldrecord->type}";
        if (!file_exists($filepath)) {
            return new callforpaper_field_base($fieldrecord, $this->instance, $this->cm);
        }
     ...
     * However, it won't update the template callforpaper field.
     *
    ...
    public function get_template(string $templatename, array $options = []): template {
        ...
        if (empty($templatecontent)) {
            $templatecontent = callforpaper_generate_default_template($instance, $templatename, 0, false, false);
        }
    ...
    /**
     * Return whether the callforpaper module requests entries approval or not.
     *
    ...
    public function can_manage_templates(?int $userid = null): bool {
        ...
        return has_capability('mod/callforpaper:managetemplates', $this->context, $userid);
    }
    ...
    public function can_export_entries(?int $userid = null): bool {
        ...
        return has_capability('mod/callforpaper:exportallentries', $this->context) ||
    ...
    public function update_templates(stdClass $newtemplates): bool {
        ...
        if (isset($record->addtemplate) && !callforpaper_tags_check($this->instance->id, $record->addtemplate)) {
                return false;
        }
    ...
    public function can_view_preset(preset $preset, ?int $userid = null): bool {
        ...
        if ($presetuserid && $presetuserid != $userid) {
            return has_capability('mod/callforpaper:viewalluserpresets', $this->context, $userid);
        }
        return true;
    }
     ...
     *
     * @return array A list with the callforpaperpreset plugins and the presets saved by users.
     */
    public function get_available_presets(): array {
        // First load the callforpaperpreset plugins that exist within the modules preset dir.
    ...
    public function get_available_saved_presets(): array {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files(CALLFORPAPER_PRESET_CONTEXT, CALLFORPAPER_PRESET_COMPONENT, CALLFORPAPER_PRESET_FILEAREA);
        if (empty($files)) {
            return $presets;
        }
        $canviewall = has_capability('mod/callforpaper:viewalluserpresets', $this->get_context());
     ...
     *
     * @return array A list with the callforpaperpreset plugins.
     */
    public static function get_available_plugin_presets(): array {
        $presets = [];

        $dirs = core_component::get_plugin_list('callforpaperpreset');
