...
/**
 * Class preset for callforpaper activity.
 *
...
class preset {
    ...
    /** @var string The preset shortname. For callforpaperpreset plugins that is the folder; for saved presets, that's the preset name. */
    public $shortname;
    ...
    public static function create_from_storedfile(manager $manager, stored_file $file): ?self {
        if ($file->get_component() != CALLFORPAPER_PRESET_COMPONENT
                || $file->get_filearea() != CALLFORPAPER_PRESET_FILEAREA
                || $file->get_contextid() != CALLFORPAPER_PRESET_CONTEXT) {
            return null;
        }
     ...
     *
     * @param manager|null $manager the current instance manager
     * @param string $pluginname the callforpaperpreset plugin name
     * @return preset|null The plugin preset or null if there is no callforpaperpreset plugin with the given name.
     */
    public static function create_from_plugin(?manager $manager, string $pluginname): ?self {
        $found = false;

        $plugins = array_keys(core_component::get_plugin_list('callforpaperpreset'));
        ...
        if (!$found) {
            // If there is no callforpaperpreset plugin with this name, return null.
            return null;
        }
    ...
    /**
     * Create a preset instance from a callforpaper_record entry, a preset name and a description.
     *
     * @param manager $manager the current instance manager
    ...
    public function export(): string {
        ...
        $exportsubdir = "mod_callforpaper/presetexport/$presetname";
        $exportdir = make_temp_directory($exportsubdir);

        // Generate and write the preset.xml file.
        $presetxmlcallforpaper = static::generate_preset_xml();
        $presetxmlfile = fopen($exportdir . '/preset.xml', 'w');
        fwrite($presetxmlfile, $presetxmlcallforpaper);
        ...
        if (! static::is_directory_a_preset($exportdir)) {
            throw new \moodle_exception('generateerror', 'callforpaper');
        }
     ...
     * @return callforpaper_field_base[]  and array with field objects
     */
    ...
    /**
     * Convert a preset.xml field callforpaper into field instance.
     *
    ...
    private function get_field_instance(
        ...
        if (class_exists($classname)) {
            ...
            if ($forpreview && !$newfield->supports_preview()) {
                $newfield = new callforpaper_field_base($fieldrecord, $instance, $cm);
            }
        } else {
            $newfield = new callforpaper_field_base($fieldrecord, $instance, $cm);
        }
    ...
    /**
     * Generate a fake field record fomr the preset.xml field callforpaper.
     *
     * @param SimpleXMLElement $fieldinfo the field xml information
     * @param int $id the field id to use
    ...
    /**
     * Returns the best name to show for a callforpaperpreset plugin.
     *
     * @param string $pluginname The callforpaperpreset plugin name.
    ...
    public static function get_name_from_plugin(string $pluginname): string {
        ...
        if (!strpos(trim($pluginname), ' ') && get_string_manager()->string_exists('modulename', 'callforpaperpreset_'.$pluginname)) {
            return get_string('modulename', 'callforpaperpreset_'.$pluginname);
        } else {
            return $pluginname;
        }
    }
    ...
    /**
     * Returns the description to show for a callforpaperpreset plugin.
     *
     * @param string $pluginname The callforpaperpreset plugin name.
    ...
    public static function get_description_from_plugin(string $pluginname): string {
        if (get_string_manager()->string_exists('modulename_help', 'callforpaperpreset_'.$pluginname)) {
            return get_string('modulename_help', 'callforpaperpreset_'.$pluginname);
        } else {
            return '';
        }
    }
     ...
     * @return stdClass A filerecord object with the callforpaperpreset context, component and filearea and the given information.
     */
    protected static function get_filerecord(string $filename, string $filepath, int $userid): stdClass {
        $filerecord = new stdClass;
        $filerecord->contextid = CALLFORPAPER_PRESET_CONTEXT;
        $filerecord->component = CALLFORPAPER_PRESET_COMPONENT;
        $filerecord->filearea = CALLFORPAPER_PRESET_FILEAREA;
        $filerecord->itemid = 0;
    ...
    public static function get_file(string $filepath, string $filename): ?stored_file {
        ...
        $fileexists = $fs->file_exists(
            CALLFORPAPER_PRESET_CONTEXT,
            CALLFORPAPER_PRESET_COMPONENT,
            CALLFORPAPER_PRESET_FILEAREA,
            0,
            $filepath,
            $filename
        );
    ...
    protected function generate_preset_xml(): string {
        ...
        $presetxmlcallforpaper = "<preset>\n\n";

        // Add description.
        $presetxmlcallforpaper .= '<description>' . htmlspecialchars($this->description ?? '', ENT_COMPAT) . "</description>\n\n";
        ...
        foreach ($rawsettings as $setting) {
            $presetxmlcallforpaper .= "<$setting>" . htmlspecialchars($instance->$setting, ENT_COMPAT) . "</$setting>\n";
        }
        ...
        $fields = $DB->get_records('callforpaper_fields', ['callforpaperid' => $instance->id]);
        ksort($fields);
        if (!empty($fields)) {
            foreach ($fields as $field) {
                $presetxmlcallforpaper .= "<field>\n";
                foreach ($field as $key => $value) {
                    if ($value != '' && $key != 'id' && $key != 'callforpaperid') {
                        $presetxmlcallforpaper .= "<$key>" . htmlspecialchars($value, ENT_COMPAT) . "</$key>\n";
                    }
                }
                $presetxmlcallforpaper .= "</field>\n\n";
            }
        ...
        $preset = new SimpleXMLElement($presetxmlcallforpaper);
    ...
    public function can_manage(): bool {
        ...
        if (has_capability('mod/callforpaper:manageuserpresets', $context)) {
            return true;
        } else {
