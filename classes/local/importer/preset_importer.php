...
/**
 * Abstract class used for callforpaper preset importers
 *
...
abstract class preset_importer {
    ...
    public function get_preset_settings(): stdClass {
        ...
        if (!preset::is_directory_a_preset($this->directory)) {
            ...
            if (empty($fileobj)) {
                throw new \moodle_exception('invalidpreset', 'callforpaper', '', $this->directory);
            }
        }
        ...
        if (
            ...
            foreach ($fieldsarray as $field) {
                ...
                $fieldstoimport->callforpaperid = $module->id;
    ...
    /**
     * Import the preset into the given callforpaper module
     *
    ...
    public function import(bool $overwritesettings): bool {
        ...
        foreach ($this->fieldstoupdate as $currentid => $updatable) {
            if ($currentid != -1 && isset($currentfields[$currentid])) {
                $fieldobject = callforpaper_get_field_from_id($currentfields[$currentid]->id, $module);
                $toupdate = false;
        ...
        foreach ($this->fieldstocreate as $newfield) {
            /* Make a new field */
            $filepath = $CFG->dirroot."/mod/callforpaper/field/$newfield->type/field.class.php";
            ...
            $classname = 'callforpaper_field_' . $newfield->type;
            $fieldclass = new $classname($newfield, $module);
            $fieldclass->insert_field();
            unset($fieldclass);
        }
        if (!empty($missingfieldtypes)) {
            echo $OUTPUT->notification(get_string('missingfieldtypeimport', 'callforpaper') . html_writer::alist($missingfieldtypes));
        }

        // Get rid of all old unused callforpaper.
        foreach ($currentfields as $cid => $currentfield) {
            if (!array_key_exists($cid, $this->fieldstoupdate)) {

                // Delete all information related to fields.
                $todelete = callforpaper_get_field_from_id($currentfield->id, $module);
        ...
        if (!empty($settings->defaultsort)) {
            if (is_numeric($settings->defaultsort)) {
                ...
                $settings->defaultsort = (int)$DB->get_field(
                    'callforpaper_fields',
                    'id',
                    ['callforpaperid' => $module->id, 'name' => $settings->defaultsort]
                );
            }
        } else {
            $settings->defaultsort = CALLFORPAPER_APPROVED;
        ...
        // Now overwrite current callforpaper settings.
        ...
        callforpaper_update_instance($module);
     ...
     * @param array $currentfields Array of current fields on callforpaper activity.
     * @return void
     */
        ...
        if (!empty($newfields)) {
            ...
            foreach ($newfields as $newid => $newfield) {
                ...
                if (array_key_exists($preservedfieldid, $preservedfields)) {
                    throw new \moodle_exception('notinjectivemap', 'callforpaper');
                }
     ...
     *
     * @return bool True if the current callforpaper needs to map the fields imported.
     */
     ...
     *
     * @param bool $overwritesettings Whether to overwrite activity settings or not.
     * @param stdClass $instance callforpaper instance object
     * @return void
     */
    public function finish_import_process(bool $overwritesettings, stdClass $instance): void {
        ...
        $backurl = new \moodle_url('/mod/callforpaper/field.php', ['d' => $instance->id]);
        redirect($backurl);
    }
     ...
     *
     * @param manager $manager the current callforpaper manager
    ...
    public static function create_from_plugin_or_directory(manager $manager, string $pluginordirectory): preset_importer {
        ...
        if (!$pluginordirectory) {
            throw new \moodle_exception('emptypresetname', 'mod_callforpaper');
        }
        try {
            ...
            throw new \moodle_exception('errorpresetnotfound', 'mod_callforpaper', '', $pluginordirectory);
        }
    }
