...
/**
 * Class template for callforpaper activity
 *
...
class template {
    ...
    /** @var array The mod_callforpaper fields. */
    protected $fields = [];
    ...
    public function __construct(manager $manager, string $templatecontent, array $options = [], ?array $fields = null) {
        ...
        $this->canmanageentries = has_capability('mod/callforpaper:manageentries', $context);
    ...
    private function parse_entry(stdClass $entry): string {
        ...
        $canmanageentry = callforpaper_user_can_manage_entry($entry, $this->instance, $context);
    ...
    private function post_parse(string $result, stdClass $entry): string {
        if ($this->ratings) {
            $result .= callforpaper_print_ratings($this->instance, $entry, false);
        }
    ...
    protected function get_tag_edit_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $url = new moodle_url('/mod/callforpaper/edit.php', $this->baseurl->params());
        $url->params([
            'rid' => $entry->id,
            'sesskey' => sesskey(),
    ...
    protected function get_tag_delcheck_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $checkbox = new checkbox_toggleall('listview-entries', false, [
            ...
            'value' => $entry->id,
            'label' => get_string('selectfordeletion', 'callforpaper'),
    ...
    protected function get_tag_userpicture_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        // If the record didn't come with user data, retrieve the user from callforpaper.
    ...
    protected function get_tag_export_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $cm = $this->manager->get_coursemodule();
        $context = $this->manager->get_context();
        $canexportall = has_capability('mod/callforpaper:exportentry', $context);
        $canexportown = has_capability('mod/callforpaper:exportownentry', $context);
        if (!$canexportall && !(callforpaper_isowner($entry->id) && $canexportown)) {
            return '';
        }
        ...
        list($formats, $files) = callforpaper_portfolio_caller::formats($this->fields, $entry);
    ...
    protected function get_tag_approve_replacement(stdClass $entry, bool $canmanageentry): string {
        global $OUTPUT;
        $context = $this->manager->get_context();
        if (!has_capability('mod/callforpaper:approve', $context) || !$this->instance->approval || $entry->approved) {
            return '';
        }
        $url = new moodle_url($this->baseurl, [
            'approve' => $entry->id,
    ...
    protected function get_tag_comments_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $cmcallforpaper = (object)[
            'context' => $context,
            'course' => $course,
            'cm' => $cm,
            'area' => 'callforpaper_entry',
            'itemid' => $entry->id,
            'showcount' => true,
            'component' => 'mod_callforpaper',
        ];
        $comment = new \core_comment\manager($cmcallforpaper);
    ...
    protected function get_tag_tags_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        return $OUTPUT->tag_list(
            core_tag_tag::get_item_tags('mod_callforpaper', 'callforpaper_records', $entry->id),
            '',
            'callforpaper-tags'
        );
    }
    ...
    protected function get_tag_actionsmenu_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        if ($this->showmore) {
            ...
            $actionmenu->add(new action_menu_link_secondary(
                $showmoreurl,
                null,
                get_string('showmore', 'mod_callforpaper')
            ));
        }
        ...
        if ($canmanageentry) {
            ...
            $editurl = new moodle_url('/mod/callforpaper/edit.php', $this->baseurl->params());
            $editurl->params([
                'rid' => $entry->id,
                'backto' => urlencode($backurl->out(false))
            ]);
        ...
        $context = $this->manager->get_context();
        if (has_capability('mod/callforpaper:approve', $context) && $this->instance->approval) {
            if ($entry->approved) {
                ...
                $actionmenu->add(new action_menu_link_secondary(
                    $disapproveurl,
                    null,
                    get_string('disapprove', 'mod_callforpaper')
                ));
            } else {
                ...
                $actionmenu->add(new action_menu_link_secondary(
                    $approveurl,
                    null,
                    get_string('approve', 'mod_callforpaper')
                ));
            }
        }
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if ($canexportall || (callforpaper_isowner($entry->id) && $canexportown)) {
                ...
                $fields = $this->manager->get_fields();
                list($formats, $files) = callforpaper_portfolio_caller::formats($fields, $entry);
                $button->set_formats($formats);
     ...
     *
     * @param stdClass|null $processeddata the previous process callforpaper information.
     * @param int|null $entryid the possible entry id
     * @param stdClass|null $entrycallforpaper the entry callforpaper from a previous form or from a real entry
     * @return string the add entry HTML content
     */
    public function parse_add_entry(
        ?stdClass $processeddata = null,
        ?int $entryid = null,
        ?stdClass $entrycallforpaper = null
    ): string {
        global $OUTPUT;
        ...
        foreach ($possiblefields as $field) {
            if ($field->field->hidden && !has_capability('mod/callforpaper:manageentries', $field->context)) {
                // Skip hidden fields for users that cannot manage entries.
                continue;
            }

            $fieldinput = $this->get_field_input($processeddata, $field, $entryid, $entrycallforpaper);
            if (strpos($templatecontent, "[[" . $field->field->name . "]]") !== false) {
        ...
        if (core_tag_tag::is_enabled('mod_callforpaper', 'callforpaper_records')) {
            $patterns[] = "##tags##";
            $replacements[] = callforpaper_generate_tag_form($entryid);
        }
     ...
     *
     * @param stdClass $processeddata the previous process callforpaper information.
     * @param callforpaper_field_base $field the field object
     * @param int|null $entryid the possible entry id
     * @param stdClass|null $entrycallforpaper the entry callforpaper from a previous form or from a real entry
     * @return string the add entry HTML content
     */
    private function get_field_input(
        ...
        if ($field->type === 'unknown') {
            ...
            $fielddisplay = $field->display_add_field($entryid, $entrycallforpaper);
        }
