...
/**
 * Class responsible for generating the action bar elements in the callforpaper module pages.
 *
...
class action_bar {
    ...
    /** @var int $cmid The callforpaper course module id. */
    private $cmid;
    ...
    public function __construct(int $id, moodle_url $pageurl) {
        $this->id = $id;
        [$course, $cm] = get_course_and_cm_from_instance($this->id, 'callforpaper');
        $this->cmid = $cm->id;
    ...
    public function get_fields_action_bar(
        ...
        $renderer = $PAGE->get_renderer('mod_callforpaper');
    ...
    public function get_fields_mapping_action_bar(): string {
        ...
        $callforpaper = $fieldsactionbar->export_for_template($renderer);
        return $renderer->render_from_template('mod_callforpaper/fields_action_bar', $callforpaper);
    }
    ...
    public function get_create_fields(bool $isprimarybutton = false): \action_menu {
        ...
        foreach ($plugins as $plugin => $fulldir) {
            $menufield[$plugin] = get_string('pluginname', "callforpaperfield_{$plugin}");
        }
        asort($menufield);
        ...
        $fieldselect->set_menu_trigger(get_string('newfield', 'mod_callforpaper'), join(' ', $triggerclasses));
        ...
        foreach ($menufield as $fieldtype => $fieldname) {
            ...
            $fieldselect->add(new \action_menu_link(
                new moodle_url('/mod/callforpaper/field.php', $fieldselectparams),
                new \image_icon('icon', $fieldtype, 'callforpaperfield_' . $fieldtype),
                $fieldname,
                false
            ));
        }
    ...
    public function get_view_action_bar(bool $hasentries, string $mode): string {
        ...
        $viewlistlink = new moodle_url('/mod/callforpaper/view.php', ['d' => $this->id]);
        $viewsinglelink = new moodle_url('/mod/callforpaper/view.php', ['d' => $this->id, 'mode' => 'single']);

        $menu = [
            $viewlistlink->out(false) => get_string('listview', 'mod_callforpaper'),
            $viewsinglelink->out(false) => get_string('singleview', 'mod_callforpaper'),
        ];
        ...
        $urlselect->set_label(get_string('viewnavigation', 'mod_callforpaper'), ['class' => 'visually-hidden']);
        $renderer = $PAGE->get_renderer('mod_callforpaper');
    ...
    public function get_templates_action_bar(): string {
        ...
        $listtemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id,
            'mode' => 'listtemplate']);
        $singletemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id,
            'mode' => 'singletemplate']);
        $advancedsearchtemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id,
            'mode' => 'asearchtemplate']);
        $addtemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'addtemplate']);
        $rsstemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'rsstemplate']);
        $csstemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'csstemplate']);
        $jstemplatelink = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'jstemplate']);
        $reviewerlisttemplate = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'reviewerlisttemplate']);
        $slottemplate = new moodle_url('/mod/callforpaper/templates.php', ['d' => $this->id, 'mode' => 'slottemplate']);

        $menu = [
            $addtemplatelink->out(false) => get_string('addtemplate', 'mod_callforpaper'),
            $singletemplatelink->out(false) => get_string('singletemplate', 'mod_callforpaper'),
            $listtemplatelink->out(false) => get_string('listtemplate', 'mod_callforpaper'),
            $reviewerlisttemplate->out(false) => get_string('reviewerlisttemplate', 'mod_callforpaper'),
            $slottemplate->out(false) => get_string('slottemplate', 'mod_callforpaper'),
            $advancedsearchtemplatelink->out(false) => get_string('asearchtemplate', 'mod_callforpaper'),
            $csstemplatelink->out(false) => get_string('csstemplate', 'mod_callforpaper'),
            $jstemplatelink->out(false) => get_string('jstemplate', 'mod_callforpaper'),
            $rsstemplatelink->out(false) => get_string('rsstemplate', 'mod_callforpaper'),
        ];

        $selectmenu = new \core\output\select_menu('presetsactions', $menu, $this->currenturl->out(false));
        $selectmenu->set_label(get_string('templatesnavigation', 'mod_callforpaper'), ['class' => 'visually-hidden']);
        ...
        $presetsactions->add(new \action_menu_link(
            $resetcurrrent,
            null,
            get_string('resettemplate', 'mod_callforpaper'),
            false,
            ['data-action' => 'resettemplate', 'data-callforpaperid' => $this->id]
        ));
        ...
        $presetsactions->add(new \action_menu_link(
            $resetallurl,
            null,
            get_string('resetalltemplates', 'mod_callforpaper'),
            false,
            ['data-action' => 'resetalltemplates', 'data-callforpaperid' => $this->id]
        ));

        $templatesactionbar = new templates_action_bar($this->id, $selectmenu, $presetsactions);
    ...
    public function get_presets_preview_action_bar(manager $manager, string $fullname, string $current): string {
        ...
        foreach (['listtemplate', 'singletemplate'] as $templatename) {
            $link = new moodle_url('/mod/callforpaper/preset.php', [
                'd' => $this->id,
        ...
        $callforpaper = [
            ...
            'backurl' => new moodle_url('/mod/callforpaper/preset.php', ['id' => $cm->id]),
            'extraurlselect' => $urlselect->export_for_template($renderer),
        ];
        return $renderer->render_from_template('mod_callforpaper/action_bar', $callforpaper);
    }
     ...
     * displayed and the callforpaper hasn't any fields.
     */
    protected function get_presets_actions_select(bool $hasimport = false): ?\action_menu {
        global $DB;

        $hasfields = $DB->record_exists('callforpaper_fields', ['callforpaperid' => $this->id]);

        // Early return if the callforpaper has no fields and the import action won't be displayed.
        ...
        if ($hasimport) {
            ...
            $actionsselect->add(new \action_menu_link(
                new moodle_url('/mod/callforpaper/preset.php', $actionsselectparams),
                null,
                get_string('importpreset', 'mod_callforpaper'),
                false,
                ['data-action' => 'importpresets', 'data-callforpaperid' => $this->cmid]
            ));
        }

        // If the callforpaper has no fields, export and save as preset options shouldn't be displayed.
        if ($hasfields) {
            // Export.
            ...
            $actionsselect->add(new \action_menu_link(
                new moodle_url('/mod/callforpaper/preset.php', $actionsselectparams),
                null,
                get_string('exportpreset', 'mod_callforpaper'),
                false
            ));
            // Save as preset.
            $actionsselect->add(new \action_menu_link(
                new moodle_url('/mod/callforpaper/preset.php', $actionsselectparams),
                null,
                get_string('saveaspreset', 'mod_callforpaper'),
                false,
                ['data-action' => 'saveaspreset', 'data-callforpaperid' => $this->id]
            ));
        }
